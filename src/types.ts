...
export type LanguageType =
  ...
  | 'c' | 'cpp' | 'objc'
  | 'documentation' | 'config' | 'data' | 'infrastructure' | 'empty'
  | 'unknown';
...
export interface ProjectIdentity {
  ...
  /** Detection confidence: high (build file + code), medium (code only), low (config/infra/docs only) */
...
export interface ProjectIndex {
  ...
  subprojects?: {  // Gate-3: monorepo sub-folder projects discovered during scan
    name: string;
    path: string;
    language: string;
    framework?: string;
...
// ============================================================
// ============================================================
// B1: System-driven execution plan
// ============================================================
/** One planned tool call with expected outcome */
export interface PlanStep {
  seq: number;
  tool: string;           // read_file | search_code | code_intel | run_command | web_search
  args: Record<string, unknown>;
  why: string;            // AI's reasoning for this step
  expectedOutcome: string; // what the AI expects to learn
  fallback?: PlanStep;    // backup if this step returns empty/error
}

/** Execution plan generated by AI — system executes step by step */
export interface ExecutionPlan {
  planId: string;
  taskDescription: string;
  steps: PlanStep[];
  expectedOutput: string;  // what final output should contain
  infoRequirements: {      // minimum information needed before synthesis
    filesToRead: string[];
    patternsToSearch: string[];
    symbolsToQuery: string[];
  };
...
/** Execution state tracked by the system loop */
export interface ExecutionState {
  plan: ExecutionPlan;
  completedSteps: StepResult[];
  pendingSteps: PlanStep[];
  collectedFiles: Map<string, string>;     // filePath → content
  collectedSymbols: Map<string, string[]>; // symbol → references
  infoGathered: {                          // what we know so far
    filesRead: Set<string>;
    patternsSearched: Set<string>;
    symbolsQueried: Set<string>;
  };
  decisionPoints: number;  // times the system triggered re-evaluation
