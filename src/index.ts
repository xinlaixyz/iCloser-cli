...
program.command('docs')
  ...
  .action(async (args: string[], options?: { json?: boolean }) => {
    ...
    try {
      ...
      // D1: ic docs ask "..." — AI answers questions from project docs
      if (action === 'ask') {
        const query = rest.join(' ');
        if (!query) { fail('用法: ic docs ask "你的问题"'); }
        const { askDocs } = await import('./core/docs-generator.js');
        const answer = await askDocs(rootPath, query, index, config);
        console.log(answer);
        return;
      }
      // D2: ic docs summarize [file] — generate document summary
      if (action === 'summarize') {
        const file = rest[0];
        if (!file) { fail('用法: ic docs summarize <文档路径>'); }
        const { summarizeDoc, readFileContent } = await import('./core/docs-generator.js');
        const content = await readFileContent(rootPath, file);
        const summary = await summarizeDoc(content, config);
        console.log(`\n${chalk.bold.cyan(file)}\n${summary}\n`);
        return;
      }
      // D8: ic docs review [file] — quality review with issues list
      if (action === 'review') {
        const file = rest[0];
        if (!file) { fail('用法: ic docs review <文档路径>'); }
        const { reviewDoc, readFileContent } = await import('./core/docs-generator.js');
        const content = await readFileContent(rootPath, file);
        const issues = await reviewDoc(content, config);
        console.log(`\n${chalk.bold.cyan(file)} 质量审查\n`);
        for (const issue of issues) {
          const icon = issue.severity === 'high' ? ICONS.fail : issue.severity === 'medium' ? ICONS.warn : ICONS.info;
          console.log(`  ${icon} ${chalk.bold(issue.section)}: ${issue.description}`);
          if (issue.suggestion) console.log(`    ${chalk.dim('→ ' + issue.suggestion)}`);
        }
        console.log();
        return;
      }
      // D9: ic docs rewrite [file] --for [audience] — adapt for different readers
      if (action === 'rewrite') {
        const forIdx = rest.indexOf('--for');
        const file = forIdx > 0 ? rest.slice(0, forIdx).join(' ') : rest.join(' ');
        const audience = forIdx > 0 ? rest.slice(forIdx + 1).join(' ') : 'beginner';
        if (!file) { fail('用法: ic docs rewrite <文档路径> --for <读者类型>'); }
        const { rewriteDoc, readFileContent } = await import('./core/docs-generator.js');
        const content = await readFileContent(rootPath, file);
        const rewritten = await rewriteDoc(content, audience, config);
        console.log(`\n${chalk.bold.cyan(file)} → ${chalk.accent(audience)}\n${rewritten}\n`);
        return;
      }
      // D7: ic docs changelog — generate changelog from git history
      if (action === 'changelog') {
        const { generateChangelog } = await import('./core/docs-generator.js');
        const changelog = await generateChangelog(rootPath, config);
        console.log(changelog);
        return;
      }

      info('用法: ic docs [status|generate|ask|summarize|review|rewrite|changelog|check|edit|diff|history|section|sync|search|link|check-consistency|toc|template]');
    } catch (err) { printError(err as Error); }
  });


// ic code (C1-C6) — AI code generation, completion, fix, refactor
program.command('code')
  .description('AI 代码操作：生成/补齐/修复/重构')
  .argument('[action...]', 'new <描述> | fix | complete <文件> | refactor <描述>')
  .option('--with-tests', '同时生成测试')
  .action(async (args, options) => {
    ...
    try {
      ...
      if (action === 'new' && rest.length > 0) {
        const { readCodePatterns, buildStyleConstraints } = await import('./core/code-writer.js');
        const patterns = await readCodePatterns(rootPath, index);
        const style = options.style ? buildStyleConstraints(index.styleFingerprint) : '';
        progress('AI 代码生成: ' + rest.join(' '));
        const resp = await provider.chat({
          systemPrompt: '你是代码生成专家。根据需求和现有代码模式生成代码。' + style + '只输出JSON变更契约。',
          context: { projectMeta: patterns, relevantCode: [], relevantMemory: '', totalTokens: 0, budgetUsed: 0 },
          task: rest.join(' ') + (options.withTests ? '同时生成测试文件' : ''),
          history: '',
        });
        ...
        for (const c of output.changes) {
          ...
          success(c.file + ' +' + c.content.split('
').length + ' 行');
        }
        return;
      }
      if (action === 'fix') {
        ...
        const lastTask = tasks.find(t => t.status === 'failed');
        if (!lastTask?.verifyResult?.errorSummary) { info('无失败验证记录'); return; }
        const errors = parseErrorOutput(lastTask.verifyResult.errorSummary);
        progress('AI 修复 ' + errors.length + ' 个错误...');
        const resp = await provider.chat({
          systemPrompt: '你是代码修复专家。只修复错误，不改其他。只输出JSON变更契约。',
          task: '错误:
...
'),
      ...
      if (action === 'complete' && rest.length > 0) {
        const fp = path.join(rootPath, rest[0]);
        const content = await (await import('./utils/fs.js')).readFile(fp);
        const { findIncompleteCode } = await import('./core/code-writer.js');
        const inc = findIncompleteCode(content);
        if (inc.length === 0) { info('未发现未完成代码'); return; }
        progress('补全 ' + inc.length + ' 处...');
        const resp = await provider.chat({
          systemPrompt: '补全函数实现。只输出JSON变更契约（完整文件内容）。',
          task: '文件: ' + rest[0] + '
' + content + '

未完成:
' + inc.map(i => 'L' + i.line + ': ' + i.signature).join('
'),
          context: { projectMeta: '', relevantCode: [], relevantMemory: '', totalTokens: 0, budgetUsed: 0 }, history: '',
        });
        const out = parseAIOutput(resp.content);
        if (out.changes.length > 0) { await (await import('./utils/fs.js')).writeFile(fp, out.changes[0].content); success(rest[0] + ' 已补全'); }
        return;
      }
      if (action === 'refactor' && rest.length > 0) {
        const { findSymbolReferences } = await import('./core/code-writer.js');
        const refs = findSymbolReferences(index, rest.join(' ').split(/s+/)[0] || rest.join(' '));
        progress('重构: ' + rest.join(' ') + ' (影响 ' + refs.length + ' 处)');
        const resp = await provider.chat({
          systemPrompt: '安全重构，保持功能不变。输出所有需修改文件的JSON变更契约。',
          task: rest.join(' ') + '
...
'),
        ...
        for (const c of parseAIOutput(resp.content).changes) {
          const fp = path.join(rootPath, c.file);
          await (await import('./utils/fs.js')).ensureDir(path.dirname(fp));
          await (await import('./utils/fs.js')).writeFile(fp, c.content);
          success(c.file + ' 已重构');
        }
        return;
      }
      info('用法: ic code [new|fix|complete|refactor]');
...
program.command('changelog')
  ...
  .action(async () => {
    ...
    try {
      ...
      const provider = createProvider({ ...config.ai, apiKey: config.ai.apiKey || '' });
      const resp = await provider.chat({
        systemPrompt: '你是发布经理。根据git log生成CHANGELOG。分类feat/fix/breaking。只输出JSON写入CHANGELOG.md。',
        task: 'Git log:
' + log + '
生成CHANGELOG.md，按版本分组。',
        context: { projectMeta: '', relevantCode: [], relevantMemory: '', totalTokens: 0, budgetUsed: 0 }, history: '',
      });
      const output = parseAIOutput(resp.content);
