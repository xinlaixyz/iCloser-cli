...
├── Episodic Memory      情景事件日志 (JSONL+SQLite索引, JSONL兜底查询)
...
- Node 24+：自动启用 `node:sqlite` 索引，提供更快的情景/语义检索。
- Node 18/20：Memory Kernel 不阻塞启动；情景记忆写入 JSONL，语义记忆写入 `rules.json/tree.md`，时间线查询从 JSONL 兜底读取。
- `ICLOSER_DISABLE_SQLITE_INDEX=1`：可显式关闭 SQLite 索引，用于 CI 或兼容性回归测试。
...
├── sqlite-store.ts     # SQLite 可选索引 (node:sqlite, episodic+semantic 表)
├── jsonl-store.ts      # JSONL 追加/读取/轮转
├── sensory-buffer.ts   # 感官缓冲区 (FIFO + 噪声过滤)
├── working-memory.ts   # 工作记忆 (16k-32k token 管理)
├── episodic.ts         # 情景记忆 (事件 CRUD + SQLite/JSONL 时间轴查询)
