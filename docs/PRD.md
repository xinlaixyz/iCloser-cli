...
AgentCode 是基于 TypeScript 的 AI coding agent CLI 系统，产品定位是 **本地工程执行器 + Claude Code / Codex 替代品 + 长期记忆系统**。它面向 Codex / Claude Code 不可用、受限或需要本地可控的环境，必须具备 Claude Code 级代码能力：理解代码库、跨文件规划和修改、运行测试与质量门禁、解释变更、支持回滚，并通过 Memory Kernel 维护项目级长期记忆，在中文开发团队和受限网络环境中稳定工作。

## 产品定位

- **组合定位**：本地工程执行器负责真实项目扫描、命令执行、验证、回滚和审计；Claude Code 替代品负责读库、改代码、解释 diff、失败修复；长期记忆系统负责沉淀项目规则、用户偏好、历史错误和团队约定。
- **替代目标**：面向无法直接使用 Codex / Claude Code / Copilot coding agent 的团队，提供可本地部署、可审计、可降级、中文友好、具备长期记忆的替代方案。
- **代码能力底线**：不能只做命令包装或本地工程执行器；必须能像 Claude Code 一样读代码、改代码、跑验证、解释 diff、处理失败并继续修复。
- **长期记忆优势**：Memory Kernel 必须让系统跨会话记住项目规则、架构决策、常见错误、用户偏好和历史任务结论，避免每次重新解释项目上下文。
- **本地可控优势**：Provider 可切换，离线 mock 可回归，Memory Kernel 可持久化项目规则，命令和文件写入必须可审计、可回滚、可限制。
- **市场验收标准**：真实 Provider 黄金路径必须跑通 `scan -> plan -> code change -> diff -> verify -> report -> rollback/commit`，并保留可复查日志。
...
规模：60+ 源码文件，含 Memory Kernel、工具执行器、任务引擎、验证器、代码生成器、CLI/REPL 等模块。
